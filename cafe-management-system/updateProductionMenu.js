...
const updateProductionMenu = async () => {
  try {
    ...
    console.log('📝 Adding new Indian menu items...');

    // Add new Indian menu items
    const menuItems = [
    ...
    await models.Menu.bulkCreate(menuItems);
    ...
    console.log(`   - Hot Beverages: 10 items`);
    console.log(`   - Cold Beverages: 10 items`);
    console.log(`   - Breakfast: 10 items`);
    console.log(`   - Lunch & Dinner: 10 items`);
    console.log(`   - Desserts: 10 items`);
    
    process.exit(0);
  } catch (error) {
    console.error('❌ Error updating production menu:', error);
    process.exit(1);
  }
};
